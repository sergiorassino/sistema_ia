...
/**
 * Permiso orden 107 — Estadística de pago de cuotas.
 *
 * Solo inserta/actualiza el catálogo en `permisos_ia` (no modifica `profesores`).
 * Equivalente a database/sql/permiso_ia_orden_107_estadistica_pago_cuotas.sql (parte INSERT).
 * Se aplica con php artisan se:migrate-legacy --force
 */
return new class extends Migration
{
    private const ORDEN = PermisosIaCatalog::ADMIN_ESTADISTICA_PAGO_CUOTAS;
    ...
    {
        if (Schema::hasTable('permisos_ia')) {
            DB::table('permisos_ia')->where('id', 107)->delete();
        }
    }
};
