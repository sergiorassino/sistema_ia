...
/**
 * Permiso orden 100 — Cierre anual: lotes y reversión.
 *
 * Solo inserta/actualiza el catálogo en `permisos_ia` (no modifica `profesores`).
 * Equivalente a database/sql/permiso_ia_orden_100_cierre_anual_lotes.sql (parte INSERT).
 * Se aplica con php artisan se:migrate-legacy --force
 */
return new class extends Migration
{
    private const ORDEN = PermisosIaCatalog::CALIF_CIERRE_ANUAL_LOTES;
    ...
    {
        if (Schema::hasTable('permisos_ia')) {
            DB::table('permisos_ia')->where('id', 100)->delete();
        }
    }
};
