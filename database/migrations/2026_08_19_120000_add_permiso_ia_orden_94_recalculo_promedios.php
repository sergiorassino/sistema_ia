...
/**
 * Permiso orden 94 — Recalcular promedios (secundario).
 *
 * Solo inserta/actualiza el catálogo en `permisos_ia` (no modifica `profesores`).
 * Equivalente a database/sql/permiso_ia_orden_94_recalculo_promedios.sql
 * Se aplica con php artisan se:migrate-legacy --force
 */
return new class extends Migration
{
    private const ORDEN = PermisosIaCatalog::CALIF_RECALCULO_PROMEDIOS;
    ...
    {
        if (Schema::hasTable('permisos_ia')) {
            DB::table('permisos_ia')->where('id', 94)->delete();
        }
    }
};
