...
/**
 * Permiso orden 104 — Copiar cursos, materias y horarios entre años lectivos.
 *
 * Solo inserta/actualiza el catálogo en `permisos_ia` (no modifica `profesores`).
 * Equivalente a database/sql/permiso_ia_orden_104_copiar_cursos_materias.sql (parte INSERT).
 * Se aplica con php artisan se:migrate-legacy --force
 */
return new class extends Migration
{
    private const ORDEN = PermisosIaCatalog::COPIAR_CURSOS_MATERIAS_ANIO;
    ...
    {
        if (Schema::hasTable('permisos_ia')) {
            DB::table('permisos_ia')->where('id', 104)->delete();
        }
    }
};
