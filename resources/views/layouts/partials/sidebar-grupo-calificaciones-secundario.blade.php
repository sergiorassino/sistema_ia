...
@if (\App\Support\Navegacion\MenuSecretariaPerfil::muestraCalificacionesSecundario())
    ...
    <div class="mt-1 space-y-0.5 se-sidebar-group-items"
        ...
        <a href="{{ route(\App\Support\CalificacionesSecundario\CalificacionesSecundarioModulos::moduloActivo(\App\Support\CalificacionesSecundario\CalificacionesSecundarioModulos::BOLETIN)
           ...
           title="{{ seSidebarTooltip(tenantBoletinSecundarioMenuEtiqueta().' v1.0') }}">
            <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                      d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
            </svg>
        ...
        <a href="{{ route($rutaPlanillaSecStaff) }}"
           ...
           title="{{ seSidebarTooltip('Planilla de calificaciones (secundario) v1.0') }}">
            <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        @endif
        @if (tienePermiso(\App\Support\PermisosIaCatalog::CALIF_RECALCULO_PROMEDIOS)
            && \App\Support\CalificacionesSecundario\CalificacionesSecundarioModulos::cargaEsEstandar())
        <a href="{{ route('calificacionesSecundario.recalculoPromedios') }}"
           @class([
               'se-sidebar-link flex items-center gap-2 px-2.5 py-2 text-[13px] rounded-md transition-colors',
               'is-active shadow-sm' => ($route ?? '') === 'calificacionesSecundario.recalculoPromedios',
           ])
           title="{{ seSidebarTooltip('Recalcular promedios (secundario) v1.0', \App\Support\PermisosIaCatalog::CALIF_RECALCULO_PROMEDIOS) }}">
            <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                      d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
            </svg>
            <span class="truncate">Recalcular promedios</span>
        </a>
        @endif
