{{-- Menú de Administración — navegación lateral. Ver layouts/administracion.blade.php --}}
        ...
        {{-- Estudiantes --}}
            ...
            <div class="mt-1 space-y-0.5 se-sidebar-group-items"
                ...
                @if (tienePermiso(\App\Support\PermisosIaCatalog::LISTADO_FAMILIAS))
                @php
                    if (! \Illuminate\Support\Facades\Route::has('listados.familias')) {
                        throw new \RuntimeException("Sidebar: falta la ruta 'listados.familias'.");
                    }
                @endphp
                <a href="{{ route('listados.familias') }}"
                   @class([
                       'se-sidebar-link flex items-center gap-2 px-2.5 py-2 text-[13px] rounded-md transition-colors',
                       'is-active shadow-sm' => request()->routeIs('listados.familias', 'listados.familias.pdf', 'listados.familias.excel'),
                   ])
                   title="{{ seSidebarTooltip('Listado de familias v1.0', \App\Support\PermisosIaCatalog::LISTADO_FAMILIAS) }}">
                    <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                              d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>
                    </svg>
                    <span class="truncate">Listado de familias</span>
                </a>
                @endif
        ...
        @endif
            ...
            <div class="mt-1 space-y-0.5 se-sidebar-group-items"
                ...
                @endif
                @if (\App\Support\PermisosCuotas::puedeEstadisticaPagoPorCurso())
                <a href="{{ route('cuotas.estadistica-pago-por-curso') }}"
                   @class([
                       'se-sidebar-link flex items-center gap-2 px-2.5 py-2 text-[13px] rounded-md transition-colors',
                       'is-active shadow-sm' => in_array($route ?? '', ['cuotas.estadistica-pago-por-curso', 'cuotas.estadistica-pago-por-curso.pdf'], true),
                   ])
                   title="{{ seSidebarTooltip('Estadística de pago por curso y cuota v1.0', \App\Support\PermisosIaCatalog::ADMIN_ESTADISTICA_PAGO_POR_CURSO) }}">
                    <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                              d="M3 10h18M3 6h18M3 14h18M3 18h18"/>
                    </svg>
                    <span class="truncate">Estadística de pago por curso</span>
                </a>
                @endif
            </div>
            @endif
            ...
            <div class="mt-1 space-y-0.5 se-sidebar-group-items"
                ...
                <div class="space-y-0.5 se-sidebar-group-items"
                    ...
                    <a href="{{ route('admin.permisos-por-tarea') }}"
                       ...
                       title="Consulta de usuarios con permiso por módulo o función v1.0">
                        <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                @if (tienePermisoConfig(\App\Support\PermisosConfiguracion::COPIAR_CURSOS_MATERIAS_ANIO))
                <a href="{{ route('config.copiar-cursos-materias') }}"
                   @class([
                       'se-sidebar-link flex items-center gap-2 px-2.5 py-2 text-[13px] rounded-md transition-colors',
                       'is-active shadow-sm' => str_starts_with($route ?? '', 'config.copiar-cursos-materias'),
                   ])
                   title="{{ seSidebarTooltip('Copiar cursos, materias y horarios de un año a otro v1.0', \App\Support\PermisosConfiguracion::COPIAR_CURSOS_MATERIAS_ANIO) }}">
                    <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <span class="truncate">Copiar cursos, materias y horarios</span>
                </a>
                @endif

                @if (tienePermisoConfig(\App\Support\PermisosConfiguracion::COPIAR_ASIGNACIONES_PROF_PRECEP))
                <a href="{{ route('config.copiar-asignaciones-profesores-preceptores') }}"
                   @class([
                       'se-sidebar-link flex items-center gap-2 px-2.5 py-2 text-[13px] rounded-md transition-colors',
                       'is-active shadow-sm' => str_starts_with($route ?? '', 'config.copiar-asignaciones-profesores-preceptores'),
                   ])
                   title="{{ seSidebarTooltip('Copiar asignación de profesores y preceptores de un año a otro v1.0', \App\Support\PermisosConfiguracion::COPIAR_ASIGNACIONES_PROF_PRECEP) }}">
                    <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                              d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>
                    </svg>
                    <span class="truncate">Copiar asignación de profesores y preceptores</span>
                </a>
                @endif

                @if (tienePermisoConfig(\App\Support\PermisosConfiguracion::PROMOVER_ALUMNOS_ANIO))
                <a href="{{ route('config.promover-alumnos') }}"
                   @class([
                       'se-sidebar-link flex items-center gap-2 px-2.5 py-2 text-[13px] rounded-md transition-colors',
                       'is-active shadow-sm' => str_starts_with($route ?? '', 'config.promover-alumnos'),
                   ])
                   title="{{ seSidebarTooltip('Promover a todos los alumnos de un año a otro v1.0', \App\Support\PermisosConfiguracion::PROMOVER_ALUMNOS_ANIO) }}">
                    <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                              d="M5 10l7-7m0 0l7 7m-7-7v18"/>
                    </svg>
                    <span class="truncate">Promover a todos los alumnos</span>
                </a>
                @endif
