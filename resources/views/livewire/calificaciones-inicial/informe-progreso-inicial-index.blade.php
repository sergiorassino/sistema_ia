{{-- Informe de Progreso Escolar (inicial): salas/cursos, selección de alumnos y PDF. --}}
<div class="mx-auto w-full max-w-5xl space-y-6">
    <section class="se-hero">
        <div class="se-hero-inner">
            <div class="min-w-0 space-y-2">
                <p class="se-eyebrow">Calificaciones · Inicial</p>
                <h2 class="text-2xl font-bold tracking-tight sm:text-3xl">Informe de progreso escolar</h2>
    ...
    <div class="se-toolbar flex-col !items-stretch gap-4 lg:flex-row lg:items-end">
        <div class="min-w-0 flex-1">
            <label for="se-ipe-inicial-curso" class="form-label">Sala / curso</label>
            <select id="se-ipe-inicial-curso" wire:model.live="cursoId" class="form-select mt-1.5 w-full max-w-xl">
        ...
        <div class="min-w-0 flex-1 lg:max-w-xs">
            <label for="se-ipe-inicial-etapa" class="form-label">Etapa del informe</label>
            <select id="se-ipe-inicial-etapa" wire:model.live="etapa" class="form-select mt-1.5 w-full">
            ...
            @if ($hayMatriculas)
                <div class="flex flex-wrap items-center justify-between gap-3 border-b border-accent-100 bg-white px-5 py-3">
                    ...
                    @if ($puedePdfLote ?? false)
                        <form method="POST"
                              action="{{ \App\Support\PortalDocente\CalificacionesInicialPortalDocente::rutaInformeProgreso('pdfLote') }}"
                              target="_blank"
                ...
                <table class="w-max max-w-full table-auto divide-y divide-accent-200 text-sm">
                    ...
                    <tbody class="divide-y divide-accent-100 bg-white">
                        @forelse ($matriculas as $mat)
                            <tr class="hover:bg-accent-50/60" wire:key="ipe-inicial-mat-{{ $mat->id }}">
                                ...
                                <td class="py-3 pl-2 pr-5 text-right align-middle">
                                    <x-pdf-post-matricula
                                        :action="\App\Support\PortalDocente\CalificacionesInicialPortalDocente::rutaInformeProgreso('pdf')"
                                        :matricula="$mat->id"
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-5 py-10 text-center text-sm text-neutral-500">
                                    No hay matrículas regulares en esta sala para el ciclo lectivo actual.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
