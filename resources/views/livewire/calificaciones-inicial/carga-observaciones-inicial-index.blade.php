{{-- Carga de observaciones (inicial): espacios curriculares agrupados por curso/sala. --}}
    ...
    <div class="se-card overflow-hidden p-0">
        <div class="border-b border-accent-200 bg-accent-50 px-5 py-3">
            ...
            <p class="text-sm text-neutral-600">
                Elija un espacio curricular para ver los alumnos del curso y cargar observaciones por etapa.
            </p>
        </div>
        ...
        @forelse ($grupos as $grupo)
            ...
            @endphp
            <div class="border-b border-accent-100 last:border-b-0" wire:key="obs-inicial-curso-{{ $curso->Id }}">
                    ...
                    <table class="se-grid-pocos-campos w-max divide-y divide-accent-200 text-sm">
                        ...
                        <tbody class="divide-y divide-accent-100 bg-white">
                            @foreach ($materias as $m)
                                <tr class="hover:bg-accent-50/60" wire:key="obs-inicial-mat-{{ $m->id }}">
                                    <td class="py-3 pl-4 pr-3 align-middle font-medium text-neutral-800">
                                        {{ $m->materia }}
                                    </td>
                                    <td class="py-3 pl-3 pr-4 text-right align-middle">
                                        <a href="{{ \App\Support\PortalDocente\CalificacionesInicialPortalDocente::route('observaciones.alumnos', ['materia' => $m->id]) }}"
