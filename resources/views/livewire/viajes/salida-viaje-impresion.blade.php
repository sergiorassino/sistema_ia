<div class="mx-auto w-full max-w-5xl space-y-6">
    <section class="se-hero">
        <div class="se-hero-inner flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div class="min-w-0 space-y-2">
                ...
                <h2 class="text-2xl font-bold tracking-tight sm:text-3xl">Imprimir autorización</h2>
    ...
    <div class="se-toolbar flex-col !items-stretch gap-4 lg:flex-row lg:items-end">
        <div class="min-w-0 flex-1">
            <label for="se-salida-viaje-curso" class="form-label">Curso</label>
            <select id="se-salida-viaje-curso" wire:model.live="cursoId" class="form-select mt-1.5 w-full max-w-xl">
        ...
        <div class="se-card overflow-hidden p-0">
            <div class="border-b border-accent-200 bg-accent-50 px-5 py-3">
                ...
                <p class="text-sm text-neutral-600">
                    Marque uno, varios o todos los estudiantes y genere el PDF de autorización.
                </p>
            </div>
            ...
            @if ($hayMatriculas)
                <div class="flex flex-wrap items-center justify-between gap-3 border-b border-accent-100 bg-white px-5 py-3">
                    ...
                    @if ($puedePdf ?? false)
                        <form method="POST"
                              action="{{ route('viajes.salidas.pdf') }}"
                              target="_blank"
                              rel="noopener noreferrer"
                              class="inline">
                            @csrf
                            <input type="hidden" name="viaje" value="{{ (int) $viajeId }}">
            ...
            <div class="flex w-full justify-center overflow-x-auto px-4 pb-4 pt-1 se-grid-angosta-wrap">
                <table class="w-max max-w-full table-auto divide-y divide-accent-200 text-sm se-grid-pocos-campos">
                    ...
                    <tbody class="divide-y divide-accent-100 bg-white">
                        @forelse ($matriculas as $mat)
                            <tr class="hover:bg-accent-50/60" wire:key="salida-viaje-mat-{{ $mat->id }}">
