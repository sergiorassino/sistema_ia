    ...
    <section class="se-hero">
        <div class="se-hero-inner flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div class="min-w-0 space-y-2">
                ...
                <h2 class="text-2xl font-bold tracking-tight sm:text-3xl">Registro de Asistencia</h2>
                <p class="text-sm text-white/80">
                    {{ schoolCtx()->nivelNombre() }} · Año lectivo {{ schoolCtx()->terlecAno() }}
                </p>
                <p class="text-xs text-white/70">
                    Modelo de este nivel: {{ $implementacionEtiqueta }}
                </p>
            </div>
            <a href="{{ route('seguimiento.registro-asistencia.feriados') }}"
    ...
    <div class="se-card overflow-hidden">
        <div class="border-b border-accent-200 bg-white px-5 py-4">
            <p class="text-sm text-neutral-700">
                Elija el mes y uno, varios o todos los cursos. Cada curso genera su propia hoja en el PDF.
                @if ($esConDatos)
                    En este nivel el registro se imprime <span class="font-semibold">con datos</span> (faltas y estadísticas).
                @else
                    En este nivel el registro se imprime <span class="font-semibold">sin datos</span> (formato para llenado manual).
                @endif
            </p>
        </div>
        ...
        <div class="bg-white px-5 py-4 space-y-6">
            <div>
                <label for="ra-mes" class="form-label">Mes del registro</label>
                <select id="ra-mes"
                        wire:model.live="mes"
                ...
                <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-neutral-600">
                            Marcá uno o más cursos del ciclo lectivo actual.
                        </p>
                    </div>
                    ...
                    <a class="btn-primary inline-flex items-center gap-2"
                       ...
                       href="{{ $pdfUrl }}">
                        ...
                        @if ($cantidadSeleccionados > 1)
                            Imprimir {{ $cantidadSeleccionados }} registros (PDF)
                        @else
                            Descargar / ver PDF
                        @endif
                    </a>
        ...
        <div>
            @if ($modalCursoAbierto)
                <div class="fixed inset-0 z-[90] flex items-center justify-center px-4 py-3 sm:px-6 sm:py-4" role="dialog" aria-modal="true"
                     aria-labelledby="ra-modal-curso-titulo">
                    ...
                    <div class="relative z-10 flex w-full max-w-lg max-h-[calc(100dvh-1.75rem)] flex-col overflow-hidden rounded-2xl bg-white shadow-xl ring-1 ring-black/5 sm:max-h-[min(calc(100dvh-2rem),30rem)]">
                        <div class="border-b border-accent-200 bg-accent-50/60 px-4 py-2.5 sm:px-5 sm:py-3">
                            <p id="ra-modal-curso-titulo" class="text-sm font-bold text-neutral-900">Elegir cursos</p>
                            <p class="mt-0.5 text-[11px] leading-snug text-neutral-600">
                                Marcá uno o varios cursos. Las selecciones fuera de la vista se mantienen al confirmar.
                            </p>
                        </div>

                        <div class="border-b border-accent-100 bg-white px-4 py-2 sm:px-5 sm:py-2.5">
                            <label for="ra-modal-curso-filtro" class="form-label">Filtrar por nombre</label>
                            <input id="ra-modal-curso-filtro"
                                   type="text"
                        ...
                        <div class="min-h-0 flex-1 overflow-y-auto px-4 py-1 sm:px-5">
                            @forelse ($modalCursoLista as $c)
                                <label wire:key="ra-modal-curso-{{ $c['id'] }}"
