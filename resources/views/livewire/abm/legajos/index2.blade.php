                ...
                <table class="min-w-full border-collapse">
                    <thead class="bg-accent-50">
                        <tr>
                            ...
                            <th class="table-header min-w-[16rem] whitespace-nowrap text-right">Acciones</th>
                        </tr>
                    </thead>
                                    ...
                                    <div class="inline-flex flex-nowrap items-center justify-end gap-2">
                                        @if (puedeModificarLegajosEstudiantes())
                                            <x-nav-contexto-estudiante
                                                ...
                                                <span data-focus-target class="btn-secondary btn-sm whitespace-nowrap">Editar</span>
                                            </x-nav-contexto-estudiante>
                                            @if (tienePermiso(\App\Support\PermisosIaCatalog::LEGAJOS_ESTUDIANTES_VER_CONTRASEÑA))
                                                <button type="button" wire:click="verPassword({{ $l->id }})" class="btn-secondary btn-sm shrink-0 whitespace-nowrap">Ver Pwrd</button>
                                            @endif
                                            <button type="button" wire:click="confirmDelete({{ $l->id }})"
                                                    class="btn-danger btn-sm shrink-0 whitespace-nowrap">Eliminar</button>
                                        @else
                                            <x-nav-contexto-estudiante
                                                destino="abm.legajos.edit"
                                                :alcance="\App\Support\Navegacion\ContextoEstudianteSesion::LEGAJO_ABM"
                                                :id-legajos="$l->id"
                                                class="inline shrink-0">
                                                <span data-focus-target class="btn-secondary btn-sm whitespace-nowrap">Ver</span>
                                            </x-nav-contexto-estudiante>
                                            @if (tienePermiso(\App\Support\PermisosIaCatalog::LEGAJOS_ESTUDIANTES_VER_CONTRASEÑA))
                                                <button type="button" wire:click="verPassword({{ $l->id }})" class="btn-secondary btn-sm shrink-0 whitespace-nowrap">Ver Pwrd</button>
                                            @endif
    ...
    @if ($showPasswordModal)
        @teleport('body')
            <div class="fixed inset-0 z-[90] flex items-center justify-center overflow-y-auto px-4 py-3 sm:px-6 sm:py-4"
                 role="dialog" aria-modal="true" aria-labelledby="legajo-estudiante-pwrd-titulo">
                ...
                <div class="relative z-10 my-auto w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-xl ring-1 ring-black/5">
                    <div class="border-b border-accent-200 px-6 py-5">
                        <h3 id="legajo-estudiante-pwrd-titulo" class="text-base font-semibold text-neutral-900">Contraseña del estudiante</h3>
                        <p class="mt-1 text-sm text-neutral-600">{{ $passwordModalEstudiante }}</p>
                    </div>
                    <div class="px-6 py-5">
