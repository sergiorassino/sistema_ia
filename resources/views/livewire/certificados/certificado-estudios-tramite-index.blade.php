{{-- Constancia de certificado de estudios en trámite — listado y modal de emisión. --}}
<div class="se-cierre-anual-fill se-matriz-list-fill"
     x-data
     x-on:certificado-estudios-tramite-abrir-pdf.window="window.open($event.detail.url, '_blank')">
    <div class="se-cierre-anual-grid se-cierre-anual-grid--matriz-listado">
        <section class="se-hero se-matriz-list-hero min-w-0 shrink-0">
            <div class="se-hero-inner flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div class="min-w-0 space-y-0.5">
                    ...
                    <h2 class="font-bold tracking-tight">Constancia de Certificado en Trámite</h2>
        ...
        <div class="se-matriz-list-toolbar se-matriz-list-toolbar--angosta">
            <div class="relative min-w-0 flex-1 sm:max-w-sm">
                ...
                <input id="buscar-cert-estu-tram"
                       type="search"
                    ...
                    <table class="se-matriz-list-tabla se-matriz-list-tabla--cert-alu-reg table-fixed">
                        ...
                        <tbody>
                            @forelse ($alumnos as $a)
                                <tr wire:key="cert-estu-tram-{{ $a['idLegajos'] }}">
    ...
    @if ($modalAbierto)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4"
             role="dialog"
             aria-modal="true"
             aria-labelledby="modal-cert-estu-tram-titulo">
            <div class="absolute inset-0 bg-neutral-900/50 backdrop-blur-sm"
                 wire:click="cerrarModal"
                 aria-hidden="true"></div>

            <div class="relative z-10 flex w-full max-w-md max-h-[calc(100dvh-1.5rem)] flex-col overflow-hidden rounded-xl border border-accent-200 bg-white shadow-xl">
                <header class="shrink-0 border-b border-accent-200 px-3.5 py-2.5">
                    <h3 id="modal-cert-estu-tram-titulo" class="text-sm font-bold leading-snug text-neutral-800">
                ...
                <form class="flex min-h-0 flex-1 flex-col" wire:submit.prevent="emitirPdf">
                    <div class="min-h-0 flex-1 space-y-2 overflow-y-auto px-3.5 py-2.5">
                        <div class="grid grid-cols-2 gap-2">
                            ...
                            <div class="min-w-0">
                                <label for="fechaEmision" class="form-label !mb-0.5">Fecha emisión</label>
                        ...
                        <div>
                            <label for="mateAdeud" class="form-label !mb-0.5">Materias adeudadas</label>
                            <textarea id="mateAdeud"
                        ...
                        <div>
                            <label for="idiomaCursado" class="form-label !mb-0.5">Idioma cursado</label>
                            <input id="idiomaCursado"
                                   type="text"
                                   wire:model="idiomaCursado"
                                   class="form-input w-full !py-1.5 !text-sm"
                                   maxlength="100"
                                   autocomplete="off">
                            @error('idiomaCursado')
                        ...
                        <div>
                            <label for="preAnte" class="form-label !mb-0.5">Presentado ante</label>
                            <input id="preAnte"
                                   type="text"
                                   wire:model="preAnte"
                                   class="form-input w-full !py-1.5 !text-sm"
                        ...
                        <label class="inline-flex cursor-pointer items-center gap-1.5 text-[11px] text-neutral-600">
                            ...
                            Guardar en certestutram al generar PDF
                        </label>
