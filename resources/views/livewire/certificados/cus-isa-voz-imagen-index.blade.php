<div class="mx-auto w-full max-w-5xl space-y-6">
    <section class="se-hero">
        <div class="se-hero-inner">
            <div class="min-w-0 space-y-2">
                ...
                <h2 class="text-2xl font-bold tracking-tight sm:text-3xl">C.U.S. / I.S.A. / Voz-Imagen</h2>
    ...
    <div class="se-toolbar flex-col !items-stretch gap-4 lg:flex-row lg:items-end">
        <div class="min-w-0 flex-1">
            <label for="se-cus-isa-curso" class="form-label">Curso</label>
            <select id="se-cus-isa-curso" wire:model.live="cursoId" class="form-select mt-1.5 w-full max-w-xl">
                ...
                <div class="flex flex-wrap items-center justify-between gap-3 border-b border-accent-100 bg-white px-5 py-3">
                    ...
                    @if ($puedePdf ?? false)
                        <div class="flex flex-wrap items-center justify-end gap-2">
                            ...
                            <form method="POST"
                                  action="{{ route('certificados.cusIsaVozImagen.pdf', ['tipo' => 'isa']) }}"
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  class="inline">
                                @csrf
                            ...
                            <form method="POST"
                                  action="{{ route('certificados.cusIsaVozImagen.pdf', ['tipo' => 'voz-imagen']) }}"
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  class="inline">
                                @csrf
                ...
                <table class="w-max max-w-full table-auto divide-y divide-accent-200 text-sm se-grid-pocos-campos">
                    ...
                    <tbody class="divide-y divide-accent-100 bg-white">
                        @forelse ($matriculas as $mat)
                            <tr class="hover:bg-accent-50/60" wire:key="cus-isa-mat-{{ $mat->id }}">
