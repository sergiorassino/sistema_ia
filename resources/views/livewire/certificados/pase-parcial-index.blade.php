{{-- Pase parcial — listado de legajos de nivel medio y modal de emisión. --}}
<div class="se-cierre-anual-fill se-matriz-list-fill"
     x-data
     x-on:pase-parcial-abrir-pdf.window="window.open($event.detail.url, '_blank')">
        ...
        <section class="se-hero se-matriz-list-hero min-w-0 shrink-0">
            <div class="se-hero-inner flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div class="min-w-0 space-y-0.5">
                    ...
                    <h2 class="font-bold tracking-tight">Pase Parcial</h2>
                    <p class="text-xs text-white/80 truncate">
                        Legajos de nivel medio · Ciclo {{ schoolCtx()->terlecAno() ?? '—' }} · Orden alfabético
                    </p>
                </div>
        ...
        <div class="se-matriz-list-card min-h-0">
            <div class="se-cierre-anual-grilla se-matriz-list-grilla se-matriz-list-grilla--unified">
                <div class="se-cierre-anual-body-wrap se-matriz-list-scroll se-grid-angosta-wrap" tabindex="0" data-se-cierre-body>
                    <table class="se-matriz-list-tabla se-matriz-list-tabla--pase-parcial table-fixed">
                        <colgroup>
                        ...
                        <tbody>
                            @forelse ($alumnos as $a)
                                <tr wire:key="pase-parcial-{{ $a['idLegajos'] }}">
    ...
    @if ($modalAbierto)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6"
             role="dialog"
             aria-modal="true"
             aria-labelledby="modal-pase-parcial-titulo">
            ...
            <div class="relative z-10 w-full max-w-lg rounded-2xl border border-accent-200 bg-white shadow-xl">
                <header class="border-b border-accent-200 px-5 py-4">
                    <h3 id="modal-pase-parcial-titulo" class="text-base font-bold text-neutral-800">
                        Solicitud de Pase Parcial
                    </h3>
                ...
                <form class="px-5 py-4 space-y-4" wire:submit.prevent="emitirPdf">
                    <div>
                        <label for="fecha-pase-parcial" class="form-label">Fecha</label>
                        <input id="fecha-pase-parcial" type="date" wire:model="fecha" class="form-input w-full">
                        @error('fecha')
                    ...
                    <div>
                        <label for="destino-pase-parcial" class="form-label">Establecimiento de destino</label>
                        <input id="destino-pase-parcial"
                               type="text"
                               wire:model="destino"
                               class="form-input w-full"
                               maxlength="200"
                               autocomplete="off"
                               placeholder="Nombre y localidad del colegio destino">
                        @error('destino')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <label class="inline-flex items-center gap-2 text-sm text-neutral-600 cursor-pointer">
                        ...
                        Guardar datos en solicitudpase al generar el PDF
                    </label>
