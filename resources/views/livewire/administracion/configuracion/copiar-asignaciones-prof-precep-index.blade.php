{{-- Copiar asignación de profesores (ppc) y preceptores por curso de un año lectivo a otro. --}}
<div class="se-page mx-auto w-full max-w-5xl space-y-6">
    <section class="se-hero">
        <div class="se-hero-inner">
            <div class="min-w-0 space-y-2">
                <p class="se-eyebrow">Configuración</p>
                <h2 class="text-2xl font-bold tracking-tight sm:text-3xl">Copiar asignación de profesores y preceptores</h2>
                <p class="max-w-2xl text-sm text-white/80">
                    Replica las asignaciones de profesores por materia y de preceptores por curso del año de origen en el año de destino. Las que ya existen no se duplican.
                </p>
            </div>
    ...
    <div class="se-card space-y-6 p-5 sm:p-6">
        <div class="grid gap-4 sm:grid-cols-2">
            <div>
                <label for="tl-origen-asig" class="form-label">Año de origen</label>
                <select id="tl-origen-asig" wire:model.live="idTerlecOrigen" class="form-select">
            ...
            <div>
                <label for="tl-destino-asig" class="form-label">Año de destino</label>
                <select id="tl-destino-asig" wire:model.live="idTerlecDestino" class="form-select">
                    <option value="">— Seleccione año —</option>
                    @foreach ($terlecs as $t)
                        <option value="{{ (int) $t->id }}">{{ $t->ano }}</option>
                    @endforeach
        ...
        @if ($preview)
            <div class="space-y-3">
                ...
                <dl class="grid grid-cols-2 gap-3">
                    @foreach ([
                        'Profesores por materia' => $preview['ppc'],
                        'Preceptores por curso' => $preview['preceptores'],
                    ] as $titulo => $bloque)
                        <div class="rounded-xl border border-accent-200 bg-accent-50/60 px-3 py-2">
                            ...
                            <dd class="mt-1 text-sm text-neutral-800">
                                ...
                                <span class="mt-0.5 block text-xs text-neutral-500">
                                    ...
                                    @if (! empty($bloque['sin_materia']))
                                        · {{ $bloque['sin_materia'] }} sin materia destino
                                    @endif
                                    @if (! empty($bloque['sin_curso']))
                                        · {{ $bloque['sin_curso'] }} sin curso destino
                                    @endif
                                </span>
                            </dd>
                        </div>
        ...
        <div>
            <button type="button"
                    ...
                    x-on:click="window.seSwalConfirmar(
                        'Se copiarán las asignaciones de profesores y de preceptores del año {{ $origenAno ?? 'origen' }} al año {{ $destinoAno ?? 'destino' }}. Las que ya existan no se duplican. Esta operación no se deshace sola.',
                        'Copiar asignaciones',
    ...
    @if (! empty($informe))
        <div class="se-card space-y-3 p-5 sm:p-6" role="status">
            ...
            <dl class="grid grid-cols-2 gap-3">
                @foreach ([
                    'Profesores por materia' => $informe['ppc'],
                    'Preceptores por curso' => $informe['preceptores'],
                ] as $titulo => $bloque)
                    <div class="rounded-xl border border-accent-200 bg-accent-50/60 px-3 py-2">
                        <dt class="text-[10px] font-semibold uppercase tracking-wide text-neutral-500">{{ $titulo }}</dt>
                        <dd class="text-lg font-semibold tabular-nums text-primary-700">{{ $bloque['a_crear'] }}</dd>
                        <dd class="text-xs text-neutral-500">
                            creados · {{ $bloque['existentes'] }} ya existían
                            @if (! empty($bloque['sin_materia']))
                                · {{ $bloque['sin_materia'] }} sin materia destino
                            @endif
                            @if (! empty($bloque['sin_curso']))
                        ...
                        <table class="w-max table-auto text-sm se-grid-pocos-campos">
                            <thead>
                                <tr class="border-b border-accent-200 bg-accent-50">
                                    ...
                                    <th scope="col" class="px-3 py-2 text-right text-[10px] font-semibold uppercase tracking-wide text-neutral-500">Profesores</th>
                                    <th scope="col" class="px-3 py-2 text-right text-[10px] font-semibold uppercase tracking-wide text-neutral-500">Preceptores</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($informe['por_nivel'] as $filaNivel)
                                    <tr class="border-b border-accent-100">
                                        ...
                                        <td class="px-3 py-2 text-right tabular-nums text-neutral-800">{{ (int) $filaNivel['ppc'] }}</td>
                                        <td class="px-3 py-2 text-right tabular-nums text-neutral-800">{{ (int) $filaNivel['preceptores'] }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                            <tfoot>
                                <tr class="border-t border-accent-200">
                                    ...
                                    <td class="px-3 py-2 text-right font-semibold tabular-nums text-primary-700">{{ (int) $informe['ppc']['a_crear'] }}</td>
                                    <td class="px-3 py-2 text-right font-semibold tabular-nums text-primary-700">{{ (int) $informe['preceptores']['a_crear'] }}</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
    ...
    <script>
        (function () {
            ...
            $wire.on('se-swal-aviso', (event) => {
                const mensaje = mensajeDeEvento(event, 'No había asignaciones nuevas para copiar.');
