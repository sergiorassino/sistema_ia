...
<div class="se-page max-w-5xl mx-auto">
    <section class="se-hero mb-4">
        <div class="se-hero-inner flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div class="min-w-0 space-y-0.5">
                ...
                <h1 class="text-xl font-bold tracking-tight text-white sm:text-2xl">Eliminar masivamente cuotas generadas</h1>
                <p class="text-xs text-white/75">
                    Ciclo lectivo {{ $ano }} · Solo estudiantes <strong class="text-white/90">regulares</strong>
                    · No se eliminan cuotas con pagos, PAGADO &gt; 0 ni FALTAPA saldado o ajustado
                </p>
            </div>
            @if ($paso > 1 && $paso < 3)
                <button type="button"
    ...
    @elseif ($paso === 1)
        <div class="se-card overflow-hidden">
            <div class="border-b border-accent-200 bg-accent-50/80 px-4 py-3 sm:px-5">
                <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <p class="text-sm text-neutral-700">
                        Elija los cursos por nivel. En el siguiente paso seleccionará la cuota generada a eliminar.
                    </p>
            ...
            <div class="border-b border-accent-200 bg-white px-4 py-3 sm:px-5">
                <label for="filtro-cursos-elim-masiva" class="form-label">Buscar curso</label>
                <input id="filtro-cursos-elim-masiva"
                       type="search"
    ...
    @elseif ($paso === 2)
        <div class="se-card overflow-hidden mb-4">
            <div class="border-b border-accent-200 bg-accent-50/80 px-4 py-3 sm:px-5">
                <p class="text-sm text-neutral-700">Seleccione la plantilla de cuota generada que desea eliminar en los cursos elegidos.</p>
            </div>
            <div class="px-4 py-4 sm:px-5">
                <label for="idCuotaElimMasiva" class="form-label">Cuota</label>
                <select id="idCuotaElimMasiva"
            ...
            <div class="flex flex-wrap gap-2 border-t border-accent-200 bg-accent-50/60 px-4 py-3 sm:px-5">
                <button type="button"
                    ...
                    <span wire:loading.remove wire:target="armarVistaPrevia">Ver cuotas a eliminar</span>
                ...
                <div class="border-b border-accent-200 bg-white px-4 py-3 sm:px-5">
                    ...
                    <p class="mt-1 text-xs text-neutral-600">
                        ...
                        @else
                            ...
                            · Se eliminará <span class="font-semibold tabular-nums">{{ $totalPrevio }}</span> cuota(s) generada(s)
                        @endif
                    </p>
                </div>
                ...
                @else
                                ...
                                <div wire:key="prev-curso-{{ $idCursoPrev }}">
                                    ...
                                    <ul class="list-none space-y-1 pl-0">
                                        @foreach ($bloque['alumnos'] ?? [] as $alumno)
                                            @php
                                                $puedeEliminar = (bool) ($alumno['puedeEliminar'] ?? false);
                                            @endphp
                                            <li wire:key="prev-{{ $alumno['idLegajo'] }}"
                                                class="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                                                <span class="text-neutral-800">{{ $alumno['etiqueta'] }}</span>
                                                <span class="{{ $puedeEliminar ? 'text-emerald-800' : 'text-amber-900' }}">
                                                    — {{ $alumno['estado'] ?? '' }}
                                                </span>
                                            </li>
                                        @endforeach
                    ...
                    <div class="flex flex-wrap justify-end gap-2 border-t border-accent-200 bg-accent-50/60 px-4 py-3 sm:px-5">
                        <button type="button"
                                ...
                                x-on:click="
                                    seSwalConfirmar(
                                        @js("Se eliminarán {$totalPrevio} cuota(s) generada(s) de «{$nombreCuota}». Esta acción no se puede deshacer."),
                                        '¿Confirma la eliminación masiva?'
                                    ).then(ok => { if (ok) $wire.eliminar(); })
                                "
                                class="inline-flex items-center rounded-xl bg-red-600 px-5 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-700 disabled:opacity-60">
                            <span wire:loading.remove wire:target="eliminar">Eliminar cuotas</span>
    ...
    @else
        @php
            ...
            $eliminadosRes = (int) ($resultado['eliminados'] ?? 0);
            $noEliminadosRes = (int) ($resultado['noEliminados'] ?? 0);
            $porCursoRes = $resultado['porCurso'] ?? [];
        @endphp

        <div class="se-card overflow-hidden mb-4">
            <div class="border-b border-accent-200 bg-accent-50/80 px-4 py-3 sm:px-5">
                <p class="text-sm font-semibold text-neutral-800">Informe de eliminación — cuota «{{ $nombreCuotaRes }}»</p>
                <p class="mt-1 text-xs text-neutral-600">
                    Eliminación exitosa: <span class="font-semibold text-emerald-800 tabular-nums">{{ $eliminadosRes }}</span>
                    @if ($noEliminadosRes > 0)
                        · No eliminados: <span class="font-semibold text-amber-900 tabular-nums">{{ $noEliminadosRes }}</span>
                    @endif
                </p>
            </div>
