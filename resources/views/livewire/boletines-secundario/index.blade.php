{{-- Módulo boletinesSecundario: informe de progreso escolar (PDF oficial). --}}
<div class="mx-auto w-full max-w-5xl space-y-6">
    ...
    <div class="se-toolbar flex-col !items-stretch gap-4 lg:flex-row lg:items-end">
        <div class="min-w-0 flex-1">
            <label for="se-boletines-curso" class="form-label">Curso</label>
            <select id="se-boletines-curso" wire:model.live="cursoId" class="form-select mt-1.5 w-full max-w-xl">
        ...
        <div class="min-w-0 flex-1 lg:max-w-xs">
            ...
            <select id="se-boletines-mostrar-promedios"
                    wire:model.live="mostrarPromedios"
                    wire:key="boletines-mostrar-promedios-{{ $mostrarPromedios }}"
            ...
            @if ($hayMatriculas)
                <div class="flex flex-wrap items-center justify-between gap-3 border-b border-accent-100 bg-white px-5 py-3">
                    ...
                    @if ($puedePdfLote ?? false)
                        <form method="POST"
                              action="{{ route('boletinesSecundario.pdfLote') }}"
                              target="_blank"
                              rel="noopener noreferrer"
                              class="inline">
                            @csrf
                            <input type="hidden" name="curso" value="{{ (int) $cursoId }}">
                            <input type="hidden" name="mostrar_promedios" value="{{ $this->debeMostrarPromedios() ? 1 : 0 }}">
                            ...
                            <tr class="hover:bg-accent-50/60" wire:key="boletin-mat-{{ $mat->id }}">
                                ...
                                <td class="py-3 pl-2 pr-5 text-right align-middle">
                                    <x-pdf-post-matricula
                                        ...
                                        :matricula="$mat->id"
                                        :fields="['mostrar_promedios' => $this->debeMostrarPromedios() ? 1 : 0]">
                                        <svg class="h-4 w-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                  d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"/>
                                        </svg>
