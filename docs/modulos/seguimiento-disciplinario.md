...
- **Registro múltiple** (botón en el header del listado): otro formulario elige curso y uno/varios/todos los estudiantes. En BD se crea **una fila `sanciones` por matrícula**. El alta, edición y listado por un alumno no cambian.
...
Migración `fechaRegistro`: `database/migrations/2026_09_22_120000_add_fecha_registro_to_sanciones_if_missing.php`.  
SQL `fechaRegistro`: `database/sql/sanciones_fecha_registro_idempotente.sql`.  
...
2. Nueva / editar sanción (formulario de campos básicos). En el alta, **Fecha de registro** es de solo lectura (fecha y hora del servidor al guardar); **Fecha** sigue siendo el día del hecho.
3. **Registro Múltiple** (header) → curso + casillas de estudiantes → mismos campos; guarda una sanción por alumno.
4. Botón **Acta** → editor enriquecido (`x-se-html-editor`); guardar con `PersistenciaColumnas`.
5. Botón **Comunicado** (solo si `enResumenComunicado = 1`) → PDF; si `acta` no está vacío, tercera sección en página nueva.
6. **Notif. Padres** → `NotificarFamiliaSancion::despachar`; si hay acta, se agrega bloque «Acta:» + texto plano al final del contenido.
...
- `app/Livewire/Seguimiento/Disciplinario/SancionRegistroMultiple.php`
- `resources/views/livewire/seguimiento/disciplinario/form-multiple.blade.php`
...
- No usar forma con barra (`Firma/s`, `Amonestación/es`): singular si cantidad = 1, plural si es 0 o ≥ 2.
- No sumar en el PDF sanciones con fecha posterior a la que se imprime.
- No reutilizar `fecha` para el momento del alta: eso va en `fechaRegistro`. En el comunicado, línea chica `Fecha de Registro: dd/mm/aaaa hh:mm` (se omite si la columna está vacía, p. ej. filas legacy).
- No pisar `fechaRegistro` al editar.
...
- [ ] ¿Registro múltiple: una fila `sanciones` por `idMatricula`, misma transacción, sin cambiar el alta individual?
...
- [ ] ¿Esquema `fechaRegistro` aplicado en el tenant (migrate o SQL)? Sin esa columna el alta de sanción falla.
