...
| `cursos` | `idNivel` | El filtro **Nivel** usa el nivel del curso de la cuota (cualquier ciclo). No define la cuenta SMTP del mail. |
...
| `ento` | `ctaEnvioMail`, `passEnvioMail`, `insti` | Cuenta SMTP **siempre** del nivel **Administración** (`idNivel` 5). |
...
- [ ] Envío SMTP solo con `MailInstitucionalConfig` del nivel Administración.
