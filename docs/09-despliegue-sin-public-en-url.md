...
| Favicon globo en menú (login bien) | Subir `app/Support/Pwa/PwaIdentity.php` y vistas `favicon`/`pwa` (sin manifiesto en layouts autenticados). Subir `public/favicon.ico` y `public/img/favicon-32.png`. `php artisan view:clear` y `config:clear`. En Red, el menú debe pedir `/ia/colegio/img/favicon-32.png`, no `/ia/colegio/pwa-personal/img/...`. |
