...
| Favicon globo en menú (login bien) | Subir `app/Support/Pwa/PwaIdentity.php`, vistas `favicon`/`pwa` y layouts (sin manifiesto en menús autenticados). Subir `public/favicon.ico` y `public/img/favicon-32.png`. `php artisan view:clear` y `config:clear`. En el menú no debe figurar `rel="manifest"` ni `favicon.ico sizes="any"`; en Red debe pedir `/ia/colegio/img/favicon-32.png`, no `/ia/colegio/pwa-personal/img/...`. |
