...
/**
 * Descargar Calificaciones desde GE (CSV) — nivel inicial.
 */
class SincroGe extends Component
{
    ...
    {
        abort_unless(tienePermiso(\App\Support\PermisosIaCatalog::CALIF_SINCRO_CIDI), 403, 'Sin permiso para descargar calificaciones desde GE.');
        abort_unless(
    ...
    public function updatedArchivoCsv(): void
    {
        ...
        if (! $this->validarEncabezadoGeInicial($this->archivoCsv)) {
            $this->addError(
                'archivoCsv',
                'El archivo no coincide con el formato GE de inicial (sala, materia y columnas de evaluaciones). Verifique que exportó el listado desde GE.'
            );
            $this->archivoCsv = null;
            $this->archivoNombre = null;
            $this->archivoTamanioKb = null;
    ...
    public function importar(GeCsvImporterPrimario $importer): void
    {
        ...
        $key = 'sincroGeInicial:import:'.(auth()->id() ?? 'guest');
        ...
        if ($idTerlec < 1 || ! NivelSistema::esInicial($idNivel)) {
            $this->addError('archivoCsv', 'No hay contexto de nivel inicial o ciclo lectivo activo en la sesión.');
    ...
    private function validarEncabezadoGeInicial(TemporaryUploadedFile $file): bool
    {
        ...
        return (str_contains($joined, 'GRADO') || str_contains($joined, 'SALA'))
    ...
    private function resolveCsvAbsolutePath(TemporaryUploadedFile $file): string
    {
        ...
        $relative = $file->storeAs(
            'imports/sincro-ge-inicial',
            'ge_ini_'.$userId.'_'.uniqid('', true).'.'.$ext,
            'local'
        );
    ...
    {
        return view('livewire.calificaciones-inicial.sincro-ge')
