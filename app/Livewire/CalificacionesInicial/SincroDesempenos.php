...
/**
 * Descargar Desempeños desde GE (CSV) — nivel inicial.
 */
class SincroDesempenos extends Component
{
    ...
    public function mount(): void
    {
        ...
        abort_unless(
            ...
            403,
            'Este módulo solo está disponible con el nivel Inicial activo en la sesión.'
        );
    }
    ...
    public function updatedArchivoCsv(): void
    {
        ...
        if (! $this->validarEncabezadoDesempenos($this->archivoCsv)) {
            $this->addError(
                'archivoCsv',
                'El archivo no coincide con el formato de desempeños (separador «;», columnas de sala, DNI y desempeño).'
            );
            $this->archivoCsv = null;
            $this->archivoNombre = null;
            $this->archivoTamanioKb = null;
    ...
    public function importar(DesempenosCsvImporter $importer): void
    {
        ...
        $key = 'sincroDesempenosInicial:import:'.(auth()->id() ?? 'guest');
    ...
    private function resolveCsvAbsolutePath(TemporaryUploadedFile $file): string
    {
        ...
        $relative = $file->storeAs(
            'imports/sincro-desempenos-inicial',
            'desemp_ini_'.$userId.'_'.uniqid('', true).'.'.$ext,
            'local'
        );
    ...
    {
        return view('livewire.calificaciones-inicial.sincro-desempenos')
