...
use App\Support\Cuotas\Siro\SiroCuponesVencidosConsulta;
use App\Support\Cuotas\Siro\SiroCuponesVencidosFiltros;
use App\Support\Cuotas\Siro\SiroCuponesVencidosRegistro;
use App\Support\PermisosCuotas;
use Illuminate\Validation\ValidationException;
use Livewire\Component;

/**
 * Actualizar cupones vencidos y subir a SIRO — filtros, grilla previa y generación de archivo.
 */
class SiroCuponesVencidosIndex extends Component
{
    /** 1 = filtros, 2 = grilla */
    public int $paso = 1;

    public string $fechaActualizarAl = '';
    ...
    {
        abort_unless(PermisosCuotas::puedeSiroCuponesVencidos(), 403);
    }
    ...
    public function aceptarFiltros(): void
    {
        ...
        try {
            $filtros = SiroCuponesVencidosFiltros::normalizarDesdeLivewire($this->filtrosCrudos());
        ...
        $fechaActualizarAl = (string) ($filtros['fechaActualizarAl'] ?? '');
        $registros = SiroCuponesVencidosConsulta::cuotasAdeudadas($filtros);
        $filas = [];
        $sube = 0;
        $noSube = 0;

        foreach ($registros as $registro) {
            $fila = SiroCuponesVencidosRegistro::filaGrilla($registro, $fechaActualizarAl);
            $filas[] = $fila;
            if ($fila['subeSiro']) {
                $sube++;
            } else {
        ...
        session([
            ...
            'siro_cupones_vencidos_ids' => collect($filas)->where('subeSiro', true)->pluck('id')->map(fn ($id) => (int) $id)->values()->all(),
        ]);
    }
    ...
    public function volverAFiltros(): void
    {
        ...
        session()->forget(['siro_cupones_vencidos_filtros', 'siro_cupones_vencidos_fecha', 'siro_cupones_vencidos_ids']);
    }
    ...
    {
        $todos = match ($tipo) {
            'cuotas' => SiroCuponesVencidosFiltros::cuotasParaSelector()->pluck('id')->map(fn ($id) => (int) $id)->all(),
            'cursos' => SiroCuponesVencidosFiltros::cursosParaSelector()->pluck('Id')->map(fn ($id) => (int) $id)->all(),
            'excluir', 'incluir' => SiroCuponesVencidosFiltros::alumnosParaSelector()->pluck('id')->map(fn ($id) => (int) $id)->all(),
            default => [],
        };
    ...
    {
        return [
            'fechaActualizarAl' => $this->fechaActualizarAl,
    ...
    public function render()
    {
        ...
        $cuotasCatalogo = SiroCuponesVencidosFiltros::cuotasParaSelector();
        $cursosCatalogo = SiroCuponesVencidosFiltros::cursosParaSelector();
        $alumnosCatalogo = SiroCuponesVencidosFiltros::alumnosParaSelector();

        return view('livewire.cuotas.siro-cupones-vencidos', [
            'ano' => $ano,
        ...
        ])->layout(layoutMenuStaff(), ['pageTitle' => "Actualizar cupones vencidos SIRO — {$ano}"]);
    }
