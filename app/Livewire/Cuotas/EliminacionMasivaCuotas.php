...
use App\Support\Cuotas\EliminacionMasivaCuotasService;
...
/**
 * Eliminación masiva de cuotas generadas por cursos (estudiantes regulares).
 */
class EliminacionMasivaCuotas extends Component
{
    ...
    {
        abort_unless(PermisosCuotas::puedeEliminacionMasiva(), 403);
    }
    ...
    public function armarVistaPrevia(): void
    {
        ...
        $this->validate([
            ...
            'idCuota.required' => 'Seleccione la cuota que desea eliminar.',
            'idCuota.min' => 'Seleccione la cuota que desea eliminar.',
        ]);
        ...
        $this->vistaPrevia = EliminacionMasivaCuotasService::vistaPrevia($cursoIds, $this->idCuota);
        $this->resultado = [];
    }
    ...
    public function eliminar(): void
    {
        ...
        $rateKey = 'cuotas:eliminar-masiva:'.(auth()->id() ?? 'guest');
        ...
        if (($this->vistaPrevia['total'] ?? 0) < 1) {
            $this->dispatch('se-swal-error', mensaje: 'No hay cuotas para eliminar. Revise la vista previa.');

            return;
        }

        $cursoIds = $this->idsCursosValidados();
        $this->resultado = EliminacionMasivaCuotasService::eliminarEnCursos($cursoIds, $this->idCuota);
        $this->paso = 3;
        $this->vistaPrevia = [];
    }
    ...
    public function render()
    {
        ...
        return view('livewire.cuotas.eliminacion-masiva', [
            'cursos' => $cursos,
