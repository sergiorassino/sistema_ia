...
use App\Support\Cuotas\Siro\SiroCuponesVencidosProceso;
...
class SiroCuponesVencidosArchivoController extends Controller
{
    public function __invoke(Request $request): Response
    {
        abort_unless(PermisosCuotas::puedeSiroCuponesVencidos(), 403);

        $key = 'siro-cupones-vencidos-archivo-dl:'.(auth()->id() ?? $request->ip());
        if (RateLimiter::tooManyAttempts($key, 10)) {
            abort(429, 'Demasiadas solicitudes. Intente nuevamente en breve.');
        }
        RateLimiter::hit($key, 60);

        $ids = session('siro_cupones_vencidos_ids', []);
        $fechaActualizarAl = trim((string) session('siro_cupones_vencidos_fecha', ''));

        if (! is_array($ids) || $ids === [] || $fechaActualizarAl === '') {
        ...
        try {
            $resultado = SiroCuponesVencidosProceso::procesar($registros, $fechaActualizarAl);
        ...
        $nombre = (string) ($archivo['nombre'] ?? 'siro-cupones-vencidos');
