...
class EstadoDeudaFamiliarListadoExcelController extends Controller
{
    public function __invoke(Request $request, string $ref): StreamedResponse
    {
        ...
        $payload = OpaqueRouteToken::decodePayload($ref, OpaqueRouteToken::PURPOSE_MORA_DEUDA_FAMILIAR_LISTADO_XLSX);
        if ($payload === null) {
            abort(404);
        }

        $key = 'mora-deuda-familiar-listado-xlsx:'.(auth()->id() ?? $request->ip());
        ...
        $resultado = EstadoDeudaFamiliarListadoExport::excel($filtros);
