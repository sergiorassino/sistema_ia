...
use App\Support\MatrizAnaliticos\AnaliticoFrenteTcpdf;
use Illuminate\Http\Request;
use Illuminate\Http\Response;
use Illuminate\Support\Facades\RateLimiter;
use Illuminate\Support\Str;

class AnaliticoFrentePdfController extends Controller
{
    public function __invoke(Request $request): Response
    {
        ...
        $key = 'analitico-frente-pdf:'.(auth()->id() ?? $request->ip()).':'.$idLegajos;
        if (RateLimiter::tooManyAttempts($key, 40)) {
            abort(429, 'Demasiadas solicitudes. Intente nuevamente en breve.');
        }
        RateLimiter::hit($key, 60);

        $datos = AnaliticoFrenteDatos::paraLegajo($idLegajos, $idNivel);
        if ($datos === null) {
            abort(404);
        }

        $pdf = AnaliticoFrenteTcpdf::generar($datos);

        $leg = $datos['legajo'];
        $slug = Str::slug(
            'analitico-frente-'.($leg['apellido'] ?? '').'-'.($leg['nombre'] ?? '').'-'.$idLegajos,
            '_',
        );
        if ($slug === '') {
            $slug = 'analitico_frente_'.$idLegajos;
        }
