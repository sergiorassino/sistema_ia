...
final class OrdenPagoTcpdf extends TCPDF
{
    ...
    /** Alto mínimo del recuadro del comprobante (no el de la hoja A4). */
    private const ALTO_CONTENIDO_MIN = 118.0;

    /** Espacio reservado bajo el encabezado: proveedor, cuerpo y pie. */
    private const ALTO_CUERPO_MIN = 58.0;
    ...
    {
        $this->datos = $datos;
        $this->altoContenido = self::altoContenidoParaDatos($datos);
    ...
    private function dibujar(): void
    {
        ...
        $x0 = self::MARGEN;
        $y0 = self::MARGEN;
        $alto = $this->altoContenido - (self::MARGEN * 2);
        ...
        $yHeaderFin = CooperadoraComprobanteEncabezadoTcpdf::dibujar(
            ...
            [
                ...
                'numero_texto' => (string) ($this->datos['orden_numero_texto'] ?? ''),
                'fecha_texto' => (string) ($this->datos['fecha_texto'] ?? ''),
                'anulado' => (bool) ($this->datos['anulado'] ?? false),
            ],
        );
     ...
     * @param  array<string, mixed>  $datos
     */
    private static function altoContenidoParaDatos(array $datos): float
    {
        ...
        $extraAnulado = ! empty($datos['anulado']) ? 6.0 : 0.0;

        return max(
            self::ALTO_CONTENIDO_MIN,
            $yHeaderFin + 2 + self::ALTO_CUERPO_MIN + $extraAnulado,
        );
    }
}
