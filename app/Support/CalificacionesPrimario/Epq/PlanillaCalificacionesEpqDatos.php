...
/**
 * Datos para la planilla de calificaciones primario EPQ (una hoja por materia).
 */
final class PlanillaCalificacionesEpqDatos
{
    /**
    ...
    public static function materiasDisponibles(bool $soloAsignadasPortal = false): Collection
    {
        ...
        } elseif (CalificacionesPrimarioPortalDocente::esPortalDocente()) {
            $idsCursos = CalificacionesPrimarioPortalDocente::idsCursosAsignados();
            if ($idsCursos === []) {
                return collect();
            }
            $query->whereIn('m.idCursos', $idsCursos);
        }
    ...
    public static function buildHojas(array $idMaterias): array
    {
        ...
        $ctx = schoolCtx();
        $permitidas = self::materiasDisponibles(CalificacionesPrimarioPortalDocente::esPortalDocente())
            ->keyBy(fn ($m) => (int) $m->id);
        ...
        foreach ($idMaterias as $idMateria) {
            ...
            $hojas[] = [
                ...
                'alumnos' => self::alumnosConNotas($idMateria, (int) $meta->idCursos, (int) $ctx->idNivel, (int) $ctx->idTerlec),
            ];
        }
    ...
    /**
     * Alumnos con fila en `calificaciones` para la materia (legacy INNER JOIN).
     *
     * @return list<array{nro: int, nombre: string, ic01: string, ic02: string, ic03: string, ic04: string, ic05: string, ic06: string, ic07: string}>
     */
    private static function alumnosConNotas(int $idMateria, int $idCurso, int $idNivel, int $idTerlec): array
    {
        $idsCondiciones = ListadoCursoCondicionFiltro::idCondicionesParaQuery(ListadoCursoCondicionFiltro::TODOS);
        $campos = CalificacionesEpqCatalogo::CAMPOS_NOTA;
    ...
    public static function etiquetaMateriaCurso(object $materia): string
    {
        $materiaTxt = trim((string) ($materia->materia ?? ''));
        $cursoTxt = trim((string) ($materia->cursec ?? ''));

        if ($cursoTxt === '') {
            return $materiaTxt;
        }

        return $materiaTxt.' · '.$cursoTxt;
    }
}
